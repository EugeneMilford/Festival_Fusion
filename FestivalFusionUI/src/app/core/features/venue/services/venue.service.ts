...
export class VenueService {
  ...
  addVenue(model: AddVenueRequest): Observable<void> {
    return this.http.post<void>(`${environment.apiBaseUrl}/api/venue`, model);
  }

  // Return all venues
  getAllVenues(): Observable<Venue[]>{
    return this.http.get<Venue[]>(`${environment.apiBaseUrl}/api/venue`);
  }

  // Get a Venue by id
  getVenueById(id: string): Observable<Venue> {
    return this.http.get<Venue>(`${environment.apiBaseUrl}/api/venue/${id}`);
  }

  // Update a Venue
  updateVenue(id: string, updateVenueRequest: UpdateVenueRequest): Observable<Venue> {
    return this.http.put<Venue>(`${environment.apiBaseUrl}/api/venue/${id}`, updateVenueRequest);
  }

  // Remove a Venue
  removeVenue(id: string): Observable<Venue> {
    return this.http.delete<Venue>(`${environment.apiBaseUrl}/api/venue/${id}`)
  }
